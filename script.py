...
class BigramLanguageModel(nn.Module):
    def __init__( self, vocab_size):
        super().__init__()
        self.token_embedding_table = nn.Embedding( vocab_size, vocab_size)  #creating an embeddings tensor (it's an object, not a collection)
    
    def forward( self, idx, targets=None):                                  #'targets=None" means this argument is optional
        logits = self.token_embedding_table( idx)                           #taking the embeddings of given context in B,T,C dimensions
...
model = BigramLanguageModel( vocab_size)    #creating model object
